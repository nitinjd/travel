...
const reportQuery = `SELECT
  ...
  GROUP_CONCAT(CONCAT(p.name,' (',p.gender,', ',p.age,
    ', bus ',CASE WHEN vo.mode='BUS' AND (p.age>=6 OR p.requires_bus_seat=1) THEN 'yes' ELSE 'no' END,
    ', accommodation ',CASE WHEN p.age>=6 OR p.requires_accommodation=1 THEN 'yes' ELSE 'no' END,
